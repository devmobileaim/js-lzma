...
this._lenDecoder.init();this._repLenDecoder.init();this._posAlignDecoder.init();this._rangeDecoder.init()};a.Decoder.prototype.decodeBody=function(b,c,f){var d=0,e=0,g=0,l=0,m=0,k=0,h=0;this._rangeDecoder.setStream(b);this._outWindow.setStream(c);for(this.init();0>f||k<f;)if(b=k&this._posStateMask,0===this._rangeDecoder.decodeBit(this._isMatchDecoders,(d<<4)+b))h=this._literalDecoder.getDecoder(k++,h),h=7<=d?h.decodeWithMatchByte(this._rangeDecoder,this._outWindow.getByte(e)):h.decodeNormal(this._rangeDecoder),
this._outWindow.putByte(h),d=4>d?0:d-(10>d?3:6);else{if(1===this._rangeDecoder.decodeBit(this._isRepDecoders,d))h=0,0===this._rangeDecoder.decodeBit(this._isRepG0Decoders,d)?0===this._rangeDecoder.decodeBit(this._isRep0LongDecoders,(d<<4)+b)&&(d=7>d?9:11,h=1):(0===this._rangeDecoder.decodeBit(this._isRepG1Decoders,d)?c=g:(0===this._rangeDecoder.decodeBit(this._isRepG2Decoders,d)?c=l:(c=m,m=l),l=g),g=e,e=c),0===h&&(h=2+this._repLenDecoder.decode(this._rangeDecoder,b),d=7>d?8:11);else if(m=l,l=g,g=
e,h=2+this._lenDecoder.decode(this._rangeDecoder,b),d=7>d?7:10,b=this._posSlotDecoder[5>=h?h-2:3].decode(this._rangeDecoder),4<=b)if(c=(b>>1)-1,e=(2|b&1)<<c,14>b)e+=a.reverseDecode2(this._posDecoders,e-b-1,this._rangeDecoder,c);else{if(e+=this._rangeDecoder.decodeDirectBits(c-4)<<4,e+=this._posAlignDecoder.reverseDecode(this._rangeDecoder),0>e){if(-1===e)break;return!1}}else e=b;if(e>=k||e>=this._dictionarySizeCheck)return!1;this._outWindow.copyBlock(e,h);k+=h;h=this._outWindow.getByte(0)}this._outWindow.flush();
...
return f};a.decompressFile=function(b,c){b instanceof ArrayBuffer&&(b=new a.iStream(b));!c&&a.oStream&&(c=new a.oStream);var f=new a.Decoder,d=f.decodeHeader(b),e=d.uncompressedSize;f.setProperties(d);if(!f.decodeBody(b,c,e))throw Error("Error in lzma data stream");return c};a.decode=a.decompressFile})(LZMA);var LZMAP=LZMAP||{};(function(a,b){b.decode=function(b,f){f=f||new a.oStream;return new Promise(function(d,e){try{a.decompressFile(b,f)}catch(g){e(g)}d(f)})}})(LZMA,LZMAP);LZMA=LZMA||{};
...
b.byteLength)this.buffers.push(b.slice(0,a));else throw Error("Buffer too small?");this.size+=a};a.oStream.prototype.toUint8Array=function(){var b=this.size,a=this.buffers;if(1==a.length)return a[0];try{for(var f=new Uint8Array(b),d=0,e=0;d<a.length;d++)f.set(a[d],e),e+=a[d].length;a[0]=f;a.length=1;return f}catch(g){console.error("Error allocating Uint8Array of size: ",b),console.error("Message given was: ",g.toString())}return null};a.oStream.prototype.forEach=function(a){for(var c=0;c<this.buffers.length;c++)a.call(this,
this.buffers[c])};a.oStream.prototype.toCodePoints=function(){a.UTF8||this.toUint8Array();return a.UTF8.decode(this.toUint8Array())};a.oStream.prototype.toString=function(){var b=this.buffers,c="";a.UTF8&&(b=[this.toCodePoints()]);for(var f=0,d=b.length;f<d;f++)for(var e=0,g=b[f].length;e<g;e++)c+=String.fromCharCode(b[f][e]);return c}})(LZMA);var UTF8=UTF8||{};
(function(a){function b(a){for(var b=0,c=a.length;c--;)128!=(a[c]&192)&&++b;return b}function c(){if(g>=e)throw Error("Invalid byte index");var a=d[g++]&255;if(128==(a&192))return a&63;throw Error("Invalid continuation byte");}function f(){var a,b,f,h;if((g|0)>(e|0))throw Error("Invalid byte index");a=d[g]&255;g=g+1|0;if(0==(a&128))return a;if(192==(a&224)){b=c();a=(a&31)<<6|b;if(128<=a)return a;throw Error("Invalid continuation byte");}if(224==(a&240)){b=c();f=c();a=(a&15)<<12|b<<6|f;if(2048<=a){if(55296<=
a&&57343>=a)throw Error("Lone surrogate U+"+a.toString(16).toUpperCase()+" is not a scalar value");return a}throw Error("Invalid continuation byte");}if(240==(a&248)&&(b=c(),f=c(),h=c(),a=(a&7)<<18|b<<12|f<<6|h,65536<=a&&1114111>=a))return a;throw Error("Invalid UTF-8 detected");}var d,e,g;a.count=b;a.decode=function(a){var c=0,k=b(a),k=new Uint16Array(k);g=0;d=a;e=a.length;for(2<(e|0)&239==(d[0]|0)&187==(d[1]|0)&191==(d[2]|0)&&(g=3);(g|0)<(e|0);)a=f()|0,k[c++]=a;return k};"undefined"!=typeof LZMA&&
(LZMA.UTF8=a)})(UTF8);
